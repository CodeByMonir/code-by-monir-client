...
export default function ColorPaletteManager() {
    ...
    return (
        <div className='mt-10'>
            <div className="w-full py-8 sm:py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-200">
                <div
                ...
                >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
                        <div>
                            ...
                            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
                                Paste all colors together using <kbd className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded text-slate-700 dark:text-slate-300 font-mono text-xs">Ctrl + V</kbd>
                            </p>
                        </div>
                    ...
                    <div className="space-y-4">
                        {colors.map((color, index) => {
                            const isValid = Boolean(parseColorToRgba(color));
                            const colorName = color ? getClosestColorName(color) : 'No color entered';

                            return (
                                <div
                                    key={index}
                                    className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 p-3.5 sm:p-4 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 rounded-xl transition"
                                >
                                    {/* Left Column: Input + Bottom Color Name Box */}
                                    ...
                                    {/* Right Side: Preview & Delete Button */}
                                    <div className="flex items-center justify-between sm:justify-start gap-3">
                                        {/* Right Card: Preview Box with checkerboard pattern for alpha transparency */}
                                        <div
                                            className="relative w-full sm:w-16 h-12 sm:h-16 rounded-xl border border-slate-300 dark:border-slate-700 shadow-inner flex items-center justify-center overflow-hidden shrink-0"
                                            style={{
                                        ...
                                        {colors.length > 1 && (
                                            <button
                                                ...
                                                className="p-1.5 text-slate-400 hover:text-rose-500 dark:text-slate-500 dark:hover:text-rose-400 hover:bg-slate-200/50 dark:hover:bg-slate-700/50 rounded-lg transition shrink-0"
